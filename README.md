...
- **void salvarDados(Passageiro passageiro[], int qtdPassageiros, Tripulacao tripulantes[], int qtdTripulantes, Voo voos[], int qtdVoos, Assento assentos[], int qtdAssentos**: Tem como objetivo salvar os dados de passageiros, tripulantes, voos e assentos em arquivos de texto, para que as informações possam ser persistidas entre execuções do programa. Ela recebe como parâmetros os vetores de dados para passageiros, tripulantes, voos e assentos, juntamente com as respectivas quantidades de elementos em cada um desses vetores.
